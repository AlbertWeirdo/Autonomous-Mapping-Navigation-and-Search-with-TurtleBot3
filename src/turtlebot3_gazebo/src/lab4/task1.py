...
class Task1(Node):
    ...
    def inflate_map(self, size):
        """
        Inflates all occupied cells (value==100) by `size` cells,
        without overwriting unknowns (value==-1).
        """
        start = time.time()
        ...
        # 1) Build a boolean mask of true obstacles
        obstacle_mask = (original == 100)

        # 2) Create a square structuring element of side (2*size+1)
        structure = np.ones((2*size + 1, 2*size + 1), dtype=bool)

        # 3) Dilate the obstacle mask
        dilated = binary_dilation(obstacle_mask, structure=structure)

        # 4) Wherever dilated==True and original was not unknown, set to occupied
        apply_mask = dilated & (original != -1)
        grid[apply_mask] = 100
    ...
    ######################################################
    def frontierSelect(self, frontiers, robot_x, robot_y, alpha=15, gamma=17):
        start = time.time()
        dist = np.inf
        goal = None
        info = 0
            ...
            if self.map_info is not None:
                ...
                if status == 0:
                    ...
                    if invalid:
                        ...
                        goal, info = self.frontierSelect(frontier_list, x ,y, alpha = 1, gamma = 0)
                    ...
                    # if np.sqrt((goal_x -self.prevgoal[0]) ** 2 + (goal_y - self.prevgoal[1]) ** 2) < 0.3 or info <= 10:
                    # if info <= 8:
                    if np.sqrt((goal_x -self.prevgoal[0]) ** 2 + (goal_y - self.prevgoal[1]) ** 2) < 0.3:
                        """
                        call wall follower and travel for 10 seconds.
                        return and 
                        """
                        cmd_vel = Twist()
                        cmd_vel.linear.x = 0.0
...
# ros2 service call /slam_toolbox/save_map slam_toolbox/srv/SaveMap "name: data: test1"